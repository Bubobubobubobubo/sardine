...
- *the external clock:* A special clock *for synchronisation on the network*. (See Diving Deeper)

Clock commands:
- ~clock.tempo~ reports the current tempo in bpm.
- ~clock.tempo = 140~ sets tempo to 140 bpm.
Position in time:
- ~clock.phase~ 0 - 1 value of current phase.
- ~clock.beat~
- ~clock.bar~
...
In addition to the internal clock, there is a special clock for synchronisation on the network.
