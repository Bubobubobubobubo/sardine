...
You don't need a text editor to play with *Sardine*. Just start *Sardine* with its own text editor by typing ~fishery web~. You can optionally the ~--port~ and  ~--host~ arguments for fine-tuning your local server. Your web browser will open as a result of typing the command. Our text editor is a web application living in your web browser, generally at ~https://localhost:8000~. The text editor is simple but has a few nice features:
- multi-pane / multi-buffer editing.
- access to the Sardine documentation.
- normal editing mode or [[https://en.wikipedia.org/wiki/Vim_(text_editor)][Vim]] mode.
...
| beat          | 1 or more beats numbers to match         | True or False |
| every         | 1 or more bars to match (modulo)         | True or False |
| phase         | low and high phase value (0.0 < x < 1.0) | True or False |
| proba         | Simple probability in %                  | True or False |

The ~beat~ function is a great place to start. Let's play a clap on the start of the bar:
#+begin_src python
Pa >> d('(if (beat 0) cp)')
#+end_src
...
These functions are cool but you might have guessed already that you can craft your own functions yourself if you are clever enough :) If you think of some cool functions to add, I'll be more than happy to include them into Sardine.
...
To install [[https://github.com/Bubobubobubobubo/ziffers-python][clone Ziffers]] from github and run pip install:
...
# Use parameters to change key and scale
zplay("q (0,4) 3 e 2 5 3 1", key="D", scale="Minor")
#+end_src
...
This is a port of the [[https://tidalcycles.org][TidalCycles]] ~sometimes~ family of functions:
- ~"always"~: 100%
- ~"almostAlways"~: 90%
- ~"often"~: 75%
- ~"sometimes"~: 50%
...
- [[http://velvetyne.fr/fonts/latitude/][Latitude]], used in headers.
