...
- You can write code directly in the interpreter. However, this is not a recommended practice!
  - You will soon begin to see that whenever you evaluate a temporal function, the system will print some useful information about it, preventing you from writing at ease in that space. You should jump to your text editor!
...
Evaluating code takes some practice. There are some pitfalls when you do live coding:
...
Why were we hearing a bassdrum and then a clap? Where does the rhythm comes from?

*Sardine* is based on the *SuperDirt* audio engine. This is *SuperDirt*'s doing. *SuperDirt* expects to receive information about audio samples and synthesizers using a simple messaging system.

*SuperDirt* expects to receive information about audio samples and synthesizers using a string (~bd~, ~cp~, etc.).

When you say ~Pa >> d('bd')~, the sound played will be the first audio sample contained in a folder called ~bd~. By default, there are a lot of samples automatically loaded by *SuperDirt* whenever you start it:
#+begin_src text
...
These synths are labelled ~super~ because they come by default with *SuperDirt*. You can write your own synthesizers using [[https://supercollider.github.io/][SuperColider]] and add them to your setup. That's what experimented players do. They have custom libraries full of synthesizers and audio samples! For now, we will stick to default.
...
This line is using a shorthand syntax for writing down patterns. This is a useful syntax to master, especially for newcomers.
...
This pattern can take *any number of arguments*. These arguments will help to preicse the timbre, rhythm and pitch of your pattern. A *Sardine* pattern is... a sandwich of values. Here is a more complex pattern based on the one we saw earlier:
#+begin_src python
...
- *pattern-relative*: *Sardine* related arguments. They will teach the system how your pattern will unfold in time (~period~, ~divisor~, ~rate~, etc).
- *instrument-relative :* *SuperDirt* / *MIDI* / *OSC* specific arguments. They will help to precise what output you would like to get out of your pattern.
...
The preceding sections of this tutorial were using a special function named ~d()~. It is now time to explain its nature. ~d()~ is what we call a *Sender*. It is a special function that takes care of sending messages to your musical applications. It also provides an interface to any pattern language that you'd like to use.
...
The following example will higlight basic usage of these *senders*:
  #+begin_src python
...
Every *sender* is specialised for a task. Every *sender* will have its own special arguments. Some of them can also be aliased (_e.g_ ~vel~ for ~velocity~). Learning these arguments is part of learning the *Sardine* instrument.
...
There is also a special ~SY~ sender that is very experimental and is used to control some very specific gear. I have currently no plan to open it for others to play but you can still send Sysex messages by using private methods of the ~midi~ object.

*** SuperDirt Arguments

There is only one *sender* for *SuperDirt*: ~D()~ or ~d()~. This sender is a basic interface to *SuperDirt*, allowing you to play sounds or synthesizers and to add effects to them.

- ~D("pattern", orbit=0)~: the *sender* for *SuperDirt*.
  - ~orbit~ : channel the sound will played on (mono or stereo).

The concept of ~orbit~ is just a way to detect on which channel of the console to apply some specific effects that will concern specifically the sound you wish to play (reverb amount, low-pass filter, etc...). This concept of ~orbit~ is specific to *SuperDirt*.

** Swimming functions

*Sardine* very existence is tied to the notion of *swimming functions*. A *swimming function* is a specific type of function that can easily be modified and updated *on the fly*, whenever you want. Strictly speaking, a *swimming function* is a *temporally recursive function*, a construct used by many programming languages for music and digital art. This way of writing functions is extremely interesting, and people have been playing with the concept for more than 30 years now. If you dig deep enough, you will find that the concept of a *swimming function* is at the very base of computer music systems such as *SuperCollider*.
...
However, this will only play once. Not so interesting for live coding or for playing on stage. A few remarks:
...
A *swimming function* is doing precisely doing this! It comes at a cost, a slightly different syntax. Evalute this:
#+begin_src python
@swim
def hello_world():
    print('Hello, World!')
...
Now it prints ~Goodbye, World!~. You can alter a swimming function whenever you can. This is really the beginning of a rabit hole of the things you can do using *swimming functions*. To illustrate that, think of the shorthand notation we mentioned earlier: ~Pa >> d('bd')~. This is a *swimming function* in disguise!

** Swimming rhythm

Rhythm in a *swimming* function using either ~p~ or the arguments : ~d, i, r~.
