...
Over its relatively short timespan, from october 2022 to december 2022, Sardine has already been used multiple times for public music performance. The thrill and danger arising from using an unstable and unpolished software has constituted an obstacle as well as an appealing perspective for most users [@roberts2018]. Jams, unformal meetings and dialogue with the wider live coding community were also of the utmost importance to study the relevance and/or the inefficiency to integrate Sardine in often already pre-established musical contexts. Special devices, such as a sender specialised in Sysex communication with a Yamaha TX7 unit (*Ralt144MI*) or with Andrew McPherson's magnetic resonator piano (*Jack Armitage*) have been developed. Hybridations between multiple live coding environments, such as *Sardine*/*TidalCycles* and *Sardine*/*Sonic Pi* have also been tried successfully (after some initial failures). Unusual use-cases were also found by Jack Armitage, who tried to run GUI refresh loops for graphical physical models using the *swimming function* mechanism. Motivation for the rewrite process was found in the perspective to support more of these creative endeavours that would be hard to undertake in more closed or less configurable live coding systems.
