...
Multiple abstractions can be built on top of the basic *swimming function* mechanism, allowing for a terser user-facing syntax. We believe that building abstraction on top of the *swimming function* is helpful to allow newcomers to get a grasp on the temporal model offered by the system. The FoxDot's inspired *surfboard* mechanism is currently the only available abstraction demonstrating this principle. It automatically handles its own scheduling logic and provides its own iterators needed by the default parser. As demonstrating by the following example, it also provides additional musical logic without the need of altering the base scheduling logic, thus adding a completely new -- and optional -- flavour of patterning/scheduling for Sardine users. Following this model, Sardine future versions are likely to include user-based modes of playing built upon the basic abstractions provided by the library.
...
\begin{center}
\textbf{Figure 10:} \textit{Sardine code displaying during an algorave at Zorba (Paris), November 3rd, 2022}
\end{center}

Over its relatively short timespan, Sardine has already been used multiple times for various musical performances. Despite the danger arising from the usage of an unstable and unpolished software during live performances (REFERENCE), we believe that it is necessary to confront the development of such a software with a creation and performance driven approach which is seen as a way to confirm the relevance of the use of the system. Jams, unformal meetings and dialogue with the wider live coding community were also of the utmost importance to study the relevance and/or the inefficiency to integrate Sardine in often already pre-established musical contexts. 
...
Despite its relative youth, having started in summer 2022, the Sardine system project is already freely usable and modifiable by its users. The project is hosted on GitHub under the GNU General Public License v.3.0. We warmly encourage anyone interested to join us and collaborate with the collective creation of a new live coding system. A Discord server and a TOPLAP `#sardine` channel are used for communication around the project. Sardine is by no mean complete or as fully featured as other existing softwares currently used by the community.
