...
from .utils import *
from .mini import *
from .stream import TidalStream, TidalListenStream
...
def tidal_listen_factory(env, osc_client, tidal_players):
    """Returns a tidal function to play Tidal patterns on a given OSC client"""
    env = env

    def tidal(key, pattern=None):
        if key not in __streams:
            __streams[key] = TidalListenStream(name=key, osc_client=osc_client)
            tidal_players.append(__streams[key])
        if pattern:
            __streams[key].pattern = pattern
        return __streams[key]
